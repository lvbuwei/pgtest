...
func makeQueries(entries map[string]string) map[string]*QueryInfo {
	m := make(map[string]*QueryInfo)
	for k, v := range entries {
		m[k] = &QueryInfo{Content: v}
	}
	return m
}
...
func TestPrepare(t *testing.T) {
	dot := DotSql{
		queries: makeQueries(map[string]string{
	...
	expectedQuery := dot.queries["select"].Content
	...
	} else if ff[0].Query != expectedQuery {
		t.Errorf("prepare was expected to be called with %q query, got %q", expectedQuery, ff[0].Query)
	}
...
func TestQuery(t *testing.T) {
	...
	if len(ff) != 1 {
		...
		t.Errorf("query was expected to be called with %q query, got %q", expectedQuery, ff[0].Query)
...
func TestExec(t *testing.T) {
	...
	if len(ff) != 1 {
		...
		t.Errorf("exec was expected to be called with %q query, got %q", expectedQuery, ff[0].Query)
	} else if len(ff[0].Args) != 1 {
...
func TestQueries(t *testing.T) {
	...
	for name, query := range got {
		if query == nil || query.Content != expectedQueryMap[name] {
			expected := expectedQueryMap[name]
			actual := ""
			if query != nil {
				actual = query.Content
			}
			t.Errorf("QueryMap()[%s] == '%s', expected '%s'", name, actual, expected)
		}
	}
}
...
func TestMergeTakesPresecendeFromLastArgument(t *testing.T) {
	...
	if expectedQuery != got.Content {
		t.Errorf("Expected query: '%s', got: '%s'", expectedQuery, got.Content)
	}
}
